...
export function Navbar() {
  ...
  const sosLabel = sosState === 'requesting_location' ? 'Locating…' : sosState === 'sending' ? 'Sending…' : sosState === 'sent' ? 'Sent' : sosState === 'error' ? 'Try again' : 'SOS';
  ...
  return (
    <>
      <aside className="fixed inset-y-0 left-0 z-50 hidden w-72 flex-col border-r border-[var(--carenet-border)] bg-[rgba(245,248,245,0.92)] px-5 py-6 shadow-[12px_0_40px_rgba(18,59,58,0.06)] backdrop-blur-xl lg:flex">
        <Link href="/" aria-label="CareNet home" className="mb-10 px-2"><CareNetLogo /></Link>
        <div className="mb-7 rounded-2xl border border-[var(--carenet-border)] bg-white/65 p-4">
        ...
        <div className="mt-auto border-t border-[var(--carenet-border)] pt-5">
      ...
      <header className="sticky top-0 z-40 border-b border-slate-200 bg-white/95 shadow-xs backdrop-blur-xl lg:ml-72">
        <div className="mx-auto flex min-h-[4.5rem] items-center justify-between gap-4 px-4 sm:px-6 lg:px-8">
          ...
          <button type="button" onClick={handleSosBroadcast} disabled={sosState === 'sending' || sosState === 'requesting_location'} aria-live="polite" className={`ml-auto flex min-h-10 items-center gap-1.5 rounded-xl px-4 text-xs font-bold transition-all shadow-sm ${sosClass} disabled:cursor-wait disabled:opacity-70`}>
            <span className="material-symbols-outlined text-[17px]" aria-hidden="true">emergency_home</span>{sosLabel}
          </button>
        </div>
        <nav className="flex gap-1 overflow-x-auto border-t border-slate-100 px-4 py-2 lg:hidden" aria-label="Mobile navigation">
          {navLinks.map(link => <Link key={link.href} href={link.href} className={`flex min-h-10 whitespace-nowrap items-center gap-1.5 rounded-lg px-3 py-1.5 text-xs font-bold transition-colors ${pathname === link.href ? 'bg-[#004340] text-white' : 'text-slate-700 hover:bg-slate-100 hover:text-slate-900'}`}><span className="material-symbols-outlined text-[16px]" aria-hidden="true">{link.icon}</span>{link.label}</Link>)}
        </nav>
      </header>
    </>
  );
